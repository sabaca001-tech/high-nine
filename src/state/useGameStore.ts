...
export const useGameStore = create<GameStore>((set, get) => ({
  ...
  closeCampReport: () => {
    const { game } = get()
    if (!game) return
    dispatch(set, game, { type: 'closeCampReport' })
  },

  chooseFriendlyMatch: (offerId) => {
    const { game } = get()
    if (!game) return
    dispatch(set, game, { type: 'chooseFriendlyMatch', offerId })
  },
  ...
  leaveScoutRegion: () => {
    const { game } = get()
    if (!game) return
    dispatch(set, game, { type: 'leaveScoutRegion' })
  },
