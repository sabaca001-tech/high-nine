 ...
 * 部費の上限。
 *
 * **999,999 では足りなかった。** 評判が上がると月の支給が5万近くになり、
 * 甲子園で勝ち上がれば1大会で数十万入る。数年でここに張り付いて、
 * それ以降は「入ってきた部費が消える」だけの状態になっていた。
 * 収入の意味が消えるので、実質頭打ちにならない桁まで上げる。
 *
 * 上限そのものを外さないのは、表示（ホーム画面の「◯k」表記）と
 * JSON の桁を壊さないための保険。
 */
export const FUNDS_MAX = 99_999_999
