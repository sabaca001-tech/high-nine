 ...
 * Sの能力は1点あたり3.4点として数えるので、
 * 突き抜けた一芸は平均を大きく押し上げる。
 *
 * **上の帯をさらに重くしてある**（S 2.5→3.4 / A 2→2.4）。
 * その代わり中〜下の帯（C以下）は軽くした。
 * 「Sが1つある選手」と「Bが並んだ選手」が同じ点数では、
 * 突き抜けた選手を採る理由が出てこない。
 *
 * | ランク | G | F | E | D | C | B | A | S |
 * |---|---|---|---|---|---|---|---|---|
 * | 以前 | 1 | 1.1 | 1.2 | 1.3 | 1.5 | 1.7 | 2 | 2.5 |
 * | いま | 1 | 1.05 | 1.12 | 1.25 | 1.45 | 1.8 | **2.4** | **3.4** |
 */
...
/**
 * 評価点の刻み（野手）。
 * 平凡な高校生（オールD＝50）で210点、
 * 全国区（オールA＝85）で690点あたりになるように置いた。
 *
 * **ランクの倍率（S 2.5→3.4）を上げたぶん、ここを下げてある**（5 → 3.4）。
 * 上の帯を重くしただけでは点数の桁が丸ごと上がって、
 * 「代表クラスで1000点」という読み方が崩れる。
 * 動かしたいのは**選手どうしの開き**であって、全体の水準ではない。
 * **投手は別（`PITCHER_POINT_SCALE`）**。球速の倍率が掛かるぶん低い。
 */
const POINT_SCALE = 3.4

/**
 * 弾道1段ぶんの評価点。**いちばん軽い能力（肩力）より軽く**なるように置く。
 * 打撃の重みを上げて肩力を軽くしたぶん、ここも下げてある（10 → 8 → 5）。
 * 刻み（`POINT_SCALE`）を下げたら、**固定点のここも一緒に下げる**
 * （下げ忘れると、弾道だけが相対的に重くなる）。
 */
const TRAJECTORY_POINTS = 5
 ...
 *
 * **打撃の価値を段階的に上げてきた**（.24 → .28 → .32 → .35）。
 * **打てるだけで主力になる**のが野球で、他が伴っていなくても構わない。
...
/*
 * **ランクの倍率と刻みを変えたら、ここも測り直す。**
 * 「全能力がそのランクちょうど」の選手が、そのランクに収まる位置に置く
 * （`rating.test.ts` が全ランクで確認している）。
 */
const POINT_THRESHOLDS: { rank: Rank; min: number }[] = [
  { rank: 'S', min: 980 }, // 全能力90で1040
  { rank: 'A', min: 650 }, // 全能力85で694
  { rank: 'B', min: 410 }, // 全能力70で428
  { rank: 'C', min: 285 }, // 全能力60で296
  { rank: 'D', min: 205 }, // 全能力50で213
  { rank: 'E', min: 145 }, // 全能力40で152
  { rank: 'F', min: 85 }, // 全能力25で89
  { rank: 'G', min: 0 },
]
