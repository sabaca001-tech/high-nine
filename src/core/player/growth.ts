 ...
 *
 * **グラウンドの効きを大きくしたぶん、ここを下げてある**（0.0135 → 0.0105）。
 * 日々の練習だけで伸びていた頃は、どこで練習しても大差が無く、
 * グラウンドの段階は「あれば少し早い」程度の飾りだった。
 *
 * **入学時の能力（`GRADE_BASE`）を上げたぶんも下げてある。**
 ...
 *
 * ただし**打ち止めにはしない。** 90以上で0.03まで絞っていた頃は、
 * 3年間かけても1つ上がるかどうかで、事実上そこで能力が凍りついていた。
 * 他校の選手は生成されるだけなのでその上限を無視して90台に並び、
 * 「育てて届く上限」と「世界に居る選手の上限」が食い違っていた。
 *
 * **伸びにくいが、伸ばし続ければ届く**水準に置き直してある。
 * 変えたのは**上の帯だけ**（80以上）。
 * 中ほどまで緩めると、チーム全体が一段強くなって
 * 甲子園に手が届きすぎる（実測で30年の優勝が6回→24回に増えた）。
 *
 * | 能力 | 40 | 50 | 60 | 70 | 75 | 80 | 85 | 90 |
 * |---|---|---|---|---|---|---|---|---|
 * | 打ち止めだった頃 | 1.0 | 1.0 | 0.7 | 0.38 | 0.24 | 0.13 | 0.07 | 0.03 |
 * | 緩めすぎた頃 | 1.0 | 1.0 | 0.7 | 0.40 | 0.28 | 0.20 | 0.15 | 0.10 |
 * | 一段絞った頃 | 1.0 | 1.0 | 0.62 | 0.33 | 0.23 | 0.16 | 0.11 | 0.07 |
 * | 前 | 0.9 | 0.9 | 0.55 | 0.28 | 0.19 | 0.13 | 0.09 | 0.06 |
 * | **いま** | **0.78** | **0.66** | **0.48** | **0.26** | **0.18** | **0.12** | **0.085** | **0.055** |
 *
 * **下の帯も段で刻んだ。** 60未満を一律にしていたので、
 * **育成の序盤〜中盤（ここにいちばん長く居る）だけ効き方が平ら**だった。
 * 段で刻んでよいのはめったに跨がない境目だけで、
 * プレイヤーが日常的に動かす帯はなだらかに効かせる。
 *
 * 打ち止め（0.03）の倍は残してあるので、伸ばし続ければ届く。
 */
function diminishingMultiplier(current: number): number {
  if (current >= 90) return 0.055
  if (current >= 85) return 0.085
  if (current >= 80) return 0.12
  if (current >= 75) return 0.18
  if (current >= 70) return 0.26
  if (current >= 60) return 0.48
  if (current >= 50) return 0.66
  if (current >= 40) return 0.78
  return 0.9
}
