...
export type Player = {
  ...
  /**
   * 学力 1〜100。**能力ではないので、スタメンや評価点には一切効かない。**
   *
   * 効くのは定期テストのときだけ。
   * 低ければ赤点で補習に取られ、高ければ要領よく両立して練習がはかどる。
   * 「勉強もしないといけない高校生」という当たり前を、
   * 判断のある形（自主学習カードを切るか）で持ち込むための値。
   */
  academics: number
  /**
   * 定期テストの結果。次のテストまで練習の伸びに掛かる。
   * 期限（`months`）が切れたら消える。
   */
  studyEffect?: { rate: number; months: number }
  /**
