...
export type Alumnus = {
   ...
   * **卒業後の伸びしろ**（0.5〜1.5あたり）。
   *
   * 大学でもプロでも、伸び幅は全員同じ乱数から引いていた。
   * 「大学で化けた」「プロでまったく伸びなかった」という差が出ず、
   * 送り出したあとの物語が誰でも同じ形になっていた。
   *
   * 高校のときの伸び代（`growthAptitude`）とは別に、卒業の時点で決める。
   * 古いセーブには無いので省略可（1.0として扱う）。
   */
  growth?: number
}

/** 卒業後の伸びしろ。省略されていれば標準（1.0） */
export function growthOf(alumnus: Alumnus): number {
